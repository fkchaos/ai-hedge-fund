...
#### 2.0 优化说明（2026-07-19更新）
本次优化深度还原了每位大佬的投资哲学和思维模型：

**优化内容：**
- agents.py: 每位大佬添加了详细的决策框架和Few-Shot示例
  - Buffett: 6步决策流程（能力圈→护城河→管理层→财务→估值→长期持有）
  - Munger: 多元思维模型（数学+心理学+经济学+生物学）
  - Lynch: 5步分析流程（分类→PEG→故事→资产负债表→增长）
  - Burry: 6步逆向分析（共识→错误→价值→催化剂→不对称性→风险）

- debater.py: 改进辩论流程
  - 优化观点格式化，更清晰易读
  - 修复初始提示词模板，添加大佬名字

- sp500_timing.py: 输出优化
  - 针对摩根标普500ETF基金(019305)的具体行动建议
  - 添加仓位建议、策略建议、止损建议

**目标：** 让用户能更好地判断摩根标普基金的买卖点
